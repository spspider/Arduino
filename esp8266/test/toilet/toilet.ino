...
#define DEFINE_GRADIENT_PALETTE 100;
#define DECLARE_GRADIENT_PALETTE 100;

#define BRIGHTNESS          100
#define FRAMES_PER_SECOND  50000
...
uint8_t col_ws8211[NUM_CHARTS];
uint8_t white_ws8211[NUM_CHARTS];

uint8_t num_ws8211 = 0;


int pos[NUM_CHARTS];
long millis_strart, millis_strart_one_sec;
long millis_10;
String Previous_set = "";
bool reverse_set;
...
void setup_ws2811() {
  ...
  String dataLoad = "{\"from\":[0],\"to\":[19],\"type\":[0],\"dir\":[0],\"col\":[0],\"num\":1,\"sp\":40,\"fd\":1,\"br\":255,\"fdt\":3}";
  //LoadData(dataLoad);
  // set master brightness control
  FastLED.setBrightness(BRIGHTNESS);
  //  String DataLoad_8211 = readCommonFiletoJson("ws8211/0");
  LoadData(dataLoad.c_str());
}
...
void LoadData(const char* json) {
  ...
  if (Previous_set.indexOf("\"r\":1") == -1) {
    ...
    Serial.println("Load data contains reverse_set");
  };
...
void one_sec() {
  ...
  } else if (duration == 0)
  {
    running_led = false;
    if (reverse_set) {
      Serial.println("reverse set:");
      //if (Previous_set.indexOf("r") == -1) {
...
void load_pattern()
{
  ...
  for (uint8_t i = 0; i < num_ws8211; i++) {
    ...
    uint8_t mid = (to_ws8211[i] - from_ws8211[i]) / 2;
    uint8_t maxLight = 192 / mid;
    uint8_t i_on = 0, i_off = 0;
    uint8_t i_div = 1;
    uint8_t divide1 = (to_ws8211[i] - from_ws8211[i]) / 3;

    switch (type_ws8211[i]) {
      case 0://с одной стороны
        pos[i] = reverseDirection(i);
        //leds[pos[i]] = CHSV( gHue, 255, 0);
        //leds[pos[i]].fadeLightBy(255);
        //fill_solid(leds, NUM_LEDS, CHSV(hue, 255, 192));
        //leds[pos[i]].maximizeBrightness(255);
      ...
      case 1://с двух сторон
        // case 5:
        pos[i] = reverseDirection(i);
        //leds[pos[i]] += CHSV( gHue, 255, 192);//бегающий огонь
        leds[pos[i]] = CHSV( gHue, 255, 192);//бегающий огонь
        //leds[pos[i]] += CHSV( gHue, 255, 192);//бегающий огонь
        uint8_t  calc_pos;
...
unsigned long millis_strart_surise;


uint8_t get_bright(uint8_t pos_point) {
...
void loop_ws2811()
{
  ...
  load_pattern();
  if (millis() > 10 + millis_strart_surise) {
    //for (uint8_t i = 0; i < 20; i++) {
    //for (uint8_t i = 0; i < 20; i++) {
    // }
    //for (uint8_t i = 0; i < 3; i++) {
    uint8_t pos_check = pos[0];
    uint8_t mid_led = pos[0];
    if ((mid_led + 1) < 20) {
      leds[mid_led + 1] = CHSV( 100, 255,  0 + bright_now[0]);
    }
    leds[mid_led] = CHSV( 100, 255, 96 + bright_now[0]); //0 -100; 2-80;3-50//
    if ((mid_led - 1) > 0) {
      leds[mid_led - 1] =  CHSV( 100, 255,  100 + bright_now[0]); //0 -100; 2-80;3-50//
    }
    if (bright_now[0] < 96) {
      bright_now[0] = bright_now[0] + 16;
    } else {
      bright_now[0] = 0;
      pos[0] = pos[0] < to_ws8211[0] ?  pos[0] + 1 : to_ws8211[0];
    }

    millis_strart_surise = millis();
  }
  ...
  if (millis() > (sp_ws8211 * 16) + millis_strart) {
    ...
    for (uint8_t i1 = 0; i1 < 3; i1++) {
      uint8_t i2;
      i2 = i1 + 1 > 20 ? 0 : i1 + 1;//3-0/2-(яркости первого)/1-(яркости второго)
      //bright_now[i1] = bright_now[i2];
    }
    //bright_now[3] = 0;
