...
short int times[Condition][Numbers];//время в формате часы*60+минуты.
    ...
    for (unsigned char i = 0; i < Numbers; i++) {
      if (En_a[idWidget][i]) {
        if (type_a[idWidget][i] == 1) {//по достижению времени
          ...
          if ((nowsec == times[idWidget][i]) || (test_action == true)) {
            test_action = false;
...
void check_if_there_timer_times(uint8_t idWidget) {//проверка установки буильника
  for (uint8_t i = 0; i < NumberIDs[idWidget]; i++) {
    if (type_a[idWidget][i] == 1) {//по достижению времени
      ...
      int Time_alarm[2];
      Time_alarm[0] = times[idWidget][i] / 60;
      Time_alarm[1] = times[idWidget][i] % 60;
    ...
    */
      ...
      if ((typePinsrepeats_int >= 0) && (typePinsrepeats_int != 255)) {
        if (!typeOpposite_int) {//включение первый раз на полив
          times[that_condtion_widget][that_number_cond] = times[that_condtion_widget][that_number_cond] + typePinsDuration_int;
        } else {
