...
void pubStatusWS(String topic_pub, String stat_pub) { 

    DynamicJsonBuffer jsonBufferParse;
    JsonObject& jsonParse = jsonBufferParse.parseObject(stat_pub);
    ...
    if ( webSocket.sendTXT(NumberWebSocket,buffer)) { 
       Serial.println("Publish new status for " + topic_pub + ", value: " + stat);
    } else {
       Serial.println("Publish new status for " + topic_pub + " FAIL!");
    }
}
