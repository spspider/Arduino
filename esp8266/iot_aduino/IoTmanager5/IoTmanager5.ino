...
int    mqttport = 16238;                                // default 1883, but CloudMQTT.com use other, for example: 13191, 23191 (SSL), 33191 (WebSockets) - use from CloudMQTT account data
...
int freeheap;
const int nWidgets = 4; // number of widgets
...
void initVar() {
  ...
  pinMode(pin[4], OUTPUT); // GPIO2 - LEN on ESP
  ...
  pin   [3] = 3;   // GPIO3 - LED attached
  sTopic[3] = prefix + "/" + deviceID + "/toggle2";
  stat  [3] = setStatus (1); // LED off at startup
  root3["id"] = 3;
  root3["page"] = "buttons";
  root3["descr"]  = "Toggle";                   // text  for description
  root3["widget"] = "toggle";
  root3["color"] = "green";                     // black, blue, green, orange, red, white, yellow (off - grey)
  root3["topic"] = sTopic[3];
  root3.printTo(thing_config[3]);
  pinMode(pin[3], OUTPUT); // GPIO3 - LEN on ESP 
}
