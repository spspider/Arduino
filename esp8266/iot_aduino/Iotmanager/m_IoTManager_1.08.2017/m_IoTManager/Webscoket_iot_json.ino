  ...
  for (int i = 0; i < nWidgets; i++) {
    if (Topic_is == sTopic[i] + "/control") {
      if ((pinmode[i] == "out") || (pinmode[i] == "in")) {
        ...
        pubStatusWS(sTopic[i], stat[i], true);
        check_if_there_timer_once(Topic_is, i, newValue);
