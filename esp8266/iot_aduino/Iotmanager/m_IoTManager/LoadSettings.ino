...
bool loadWidgets() {
  File configFile = SPIFFS.open("/pin_setup_real.txt", "r");
  if (!configFile) {
    // если файл не найден
    Serial.println("Failed to open pin_setup_real.txt");
    configFile.close();
    return false;
  }
  // Проверяем размер файла, будем использовать файл размером меньше 1024 байта
...
bool saveIRsetup(String json) {
  ...
  if (!configFile) {
    Serial.println("Failed to open pin_setup_real.txt for writing");
    return false;
  }
