...
IPAddress remoteIPmy(192, 168, 1, 101);
...
#define SWAP(x,y) swap = x; x = y; y = swap
  ...
  Serial.print(F("\t"));
  //Serial.println(F(""));
  Udp.beginPacket(remoteIPmy, Udp.remotePort());
  //Udp.beginPacket(Udp.remoteIP(), Udp.remotePort());
  Udp.print("{X:");
  Udp.print(accel_t_gyro.value.x_gyro);
  Udp.print(",Y:");
  Udp.print(accel_t_gyro.value.y_gyro);
  Udp.print(",Z:");
  Udp.print(accel_t_gyro.value.z_gyro);
  Udp.print("}");
  Udp.endPacket();
  delay(500);
  loop_udp();
