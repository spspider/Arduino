...
void captive_setup() {
  ...
  if ( load_ssid_pass()) {
    connect_captive= strlen(ssid) > 0; // Request WLAN connect if there is a SSID
    if (connect_captive) {
...
void captive_loop() {
  if (connect_captive) {//если есть SSID в EEPROM
    status = WL_IDLE_STATUS;//обнуляем статус, для подключения
  ...
  }
  {
    ...
    if ( millis() > lastConnectTry + 60L * 1000L ) {
