...
void try_connect_free() {
  ...
  int n = WiFi.scanNetworks();//сканируем Wifi сети
  if (n > 0) {//если сетей больше одной
    ...
    if (number > 0) {
      long ssid_number = random(number - 1);
...
void captive_loop() {
  ...
  }
  {
    ...
    int s = WiFi.status();
    if ( millis() > lastConnectTry + 30000 ) {
    ...
    if (ESP_busy) {
      if (millis() > buff_ESP_busy + 30000 ) {
        ESP_busy = false;
      }
    } else {
      buff_ESP_busy = millis();
    }
      ...
      if (s == WL_CONNECTED) {
        ...
        if (!MDNS.begin(myHostname)) {
          ...
          if (geo_enable) {
            ...
            saveCommonFiletoJson("ip_gps", pos);
            sendEmail(pos);
          }
          ///////////
        }
        ...
        try_MQTT_access = false;
        //WiFi.disconnect();
        if (internet_pin != 255) {//если подключение через реле может реле инвертированно
          internet_pin_inv = digitalRead(internet_pin); //читаем текущее состояние
